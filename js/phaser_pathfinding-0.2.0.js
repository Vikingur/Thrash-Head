	...
	**/
	this.findPath = function(startX, startY ,endX, endY, callback) {
		...
		endX > collisionGrid[0].length-1 || endY > collisionGrid.length-1) {
			var error = "Your start or end point is outside the scope of your grid." + (startX)+ " "+ (startY) +" "+(endX) + " "+(endY) + " "+(collisionGrid[0].length) + " "+(collisionGrid.length);
			throw error;
		}

		//Start and end are the same tile.
		if (startX===endX && startY===endY) {
			throw new Error("Empty path X:" + startX + " "+endX+" "+startY+ " "+endY);
			callback([]);
		}
		...
		if (isAcceptable === false) {
			throw new Error("This deli tray is unacceptable!\n+"+collisionGrid[endY][endX]+"\nX: "+endX+" Y: "+endY);
			callback(null);
			return;
		}
	...
	**/
	this.calculate = function() {
		if (instances.length === 0 || collisionGrid === undefined || acceptableTiles === undefined) {
			throw new Error("The first one!");
			return;
		}
		for (iterationsSoFar = 0; iterationsSoFar < iterationsPerCalculation; iterationsSoFar++) {
			if (instances.length === 0) {
//				throw new Error("The second one! "+iterationsSoFar);
				return;
			}
